...
- (id)executeScript
{
    return [self executeScriptWithContext:[Definitions globalContext]];
}
...
- (BOOL)isEqual:(id)obj
{
    if ([[obj className] isEqual:@"ScriptToken"]) {
...
{
    if ([[instruction className] isEqual:@"ScriptToken"]) {
            ...
            if ([text hasSuffix:@":"]) {
                ...
                if ([_selectorName length]) {
                    ...
                    if (!result) {
                        ...
                        if ([_contextualObject isDictionary]) {
