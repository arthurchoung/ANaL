...
{
    [Definitions runWindowManager:@"enterWorkbenchMode"];
}
...
- (void)raiseObjectWindow:(id)dict
{
    ...
    if ([dict intValueForKey:@"ANALNOFRAME"]) {
        id childWindow = [dict valueForKey:@"childWindow"];
        if (childWindow) {
...
NSLog(@"setFocusDict:%@", dict);
    {
        ...
        } else if ([dict intValueForKey:@"ANALNOFRAME"]) {
            id childWindow = [dict valueForKey:@"childWindow"];
...
NSLog(@"changes x %d y %d width %d height %d", e->x, e->y, e->width, e->height);
        if ([self doesWindow:e->window haveProperty:"ANALNOFRAME"]) {
        } else {
            return;
        }
    }
...
NSLog(@"handleX11MapRequest parent %x window %x", e->parent, e->window);
    ...
    BOOL noframe = [self doesWindow:e->window haveProperty:"ANALNOFRAME"];
    ...
    if (attrs.x == 0) {
        if (attrs.y == 0) {
            ...
            id cmd = nsarr();
            [cmd addObject:@"anal-getCoordinatesForNewWindow.pl"];
...
//        [dict setValue:name forKey:@"name"];
        [dict setValue:@"1" forKey:@"ANALNOFRAME"];
...
- (unsigned long)openAppMenuWindowsForArray:(id)array
{
    ...
    for (int i=[array count]-1; i>=0; i--) {
        ...
        if (dict) {
            unsigned long win = [dict unsignedLongValueForKey:@"window"];
            [self XChangeProperty:win name:"ANALAPPMENUNEXT" str:nsfmt(@"%lu", prev)];
            prev = win;
        }
    }
    return prev;
...
- (id)getAppMenuForWindow:(unsigned long)win
{
    ...
    id keys = nsdict();
    id arr = nsarr();
    id value = [self XGetWindowProperty:win name:"ANALAPPMENUHEAD"];
    for(;;) {
        if (!value) {
            break;
        }
        ...
        [arr addObject:dict];
        value = [self XGetWindowProperty:win name:"ANALAPPMENUNEXT"];
    }
    if ([arr count]) {
        return arr;
    }
