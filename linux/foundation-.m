...
static FILE *ANAL_stdout = NULL;

#define OUT(...) ((void)fprintf(ANAL_stdout, __VA_ARGS__))

static FILE *ANAL_stderr = NULL;

#define ERR(...) ((void)fprintf(ANAL_stderr, __VA_ARGS__))
...
void NSOut(id formatString, ...)///$;
{
    ...
    vfprintf(ANAL_stdout, [formatString UTF8String], args);
    va_end(args);
#else
...
void NSErr(id formatString, ...)///$;
{
    ...
    va_list args;
    va_start(args, formatString);
    vfprintf(ANAL_stderr, [formatString UTF8String], args);
    va_end(args);
#else
    char *fmt = [formatString UTF8String];
    char *strp = NULL;
...
void ANAL_initialize_stdout(FILE *fp)
{
    ANAL_stdout = fp;
}

void ANAL_initialize(FILE *fp)
{
    ANAL_stderr = fp;
...
fprintf(ANAL_stderr, "ERROR! register_printf_function\n");
        exit(0);
    }
#endif
